...
import { mockUserEmailModification, mockUserRoleModification, mockUsers, mockUserDetails, mockEntityDetails, resetUsers, currentUsers, setCurrentData } from "./helpers/mocks";
...
test("renders table with correct columns", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "Role" }).locator('div')).toBeVisible();
...
test("validates email input while editing user", async ({ page }) => {
  ...
  await mockEntityDetails(page, USER_DETAILS_PATH, currentUsers, userDetilsMock, USER_ENTITY);
...
test("restores user details while updating", async ({ page }) => {
  ...
  await page.getByRole('dialog', { name: 'Edit user' }).getByRole('button').nth(2).click();
