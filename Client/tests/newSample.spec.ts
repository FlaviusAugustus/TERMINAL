...
import { mockSampleCreation, mockRecipes, mockRecipesAmount, mockRecipeDetails, mockParameters, mockTags, currentRecipes } from "./helpers/mocks";
...
async function prepareSample(page: any, sample: any) {
  ...
  await page.getByRole('tabpanel', { name: 'Step' }).locator('input[type="text"]').click();
  await page.getByRole('tabpanel', { name: 'Step' }).locator('input[type="text"]').fill('60');
...
test("adds new sample with recipe successfully", async ({ page }) => {
  await mockRecipes(page, currentRecipes, "**/api/recipes?pageNumber=0&pageSize=1&desc=true");
  ...
  await page.locator('div').filter({ hasText: /^Temperaturevalue drag-handle-lineunitC⁰$/ }).getByRole('textbox').click();
  await page.locator('div').filter({ hasText: /^Temperaturevalue drag-handle-lineunitC⁰$/ }).getByRole('textbox').fill('420');
  await page.getByRole('textbox', { name: 'Comment' }).click();
  await page.getByRole('textbox', { name: 'Comment' }).fill('First step modified');
  await sample.submit();
});
