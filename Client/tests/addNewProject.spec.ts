...
test('should add new project successfully', async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: 'New-Project-Test' })).toBeVisible();
});
...
test('should trim name while adding the project', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Project', exact: true })).toBeVisible();
    ...
    await expect(page).toHaveURL(/.*new-project/);
    await expect(page.getByText('Add new project').nth(1)).toBeVisible();
  await page.getByRole('textbox', { name: 'Name:' }).fill(' Trim-Project-Test ');
    await page.getByRole('button', { name: 'Add Project' }).click();
    await expect(page.getByText('Project added succesfully')).toBeVisible();
  await page.goto('/projects');
  await expect(page.getByRole('cell', { name: 'Trim-Project-Test' })).toBeVisible();
});
...
test('should show validation error for invalid project name', async ({ page }) => {
    await page.getByRole('button', { name: 'Add new', exact: true }).click();
  await expect(page.getByRole('link', { name: 'Project', exact: true })).toBeVisible();
    await page.getByRole('link', { name: 'Project', exact: true }).click();
    await expect(page).toHaveURL(/.*new-project/);
