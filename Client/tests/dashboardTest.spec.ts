...
test("should redirect to Projects page on clicking Browse All", async ({
  page,
}) => {
...
test("should redirect to Add new project page on clicking Add New", async ({
  page,
}) => {
...
test("should redirect to Samples page on clicking Browse All under Samples", async ({
  page,
}) => {
...
test("should redirect to Add new sample page on clicking Add New under Samples", async ({
  page,
}) => {
...
test("should redirect to Recipes page on clicking Browse All under Recipes", async ({
  page,
}) => {
...
test("should redirect to Add new recipe page on clicking Add New under Recipes", async ({
  page,
}) => {
...
test("should redirect to Users page on clicking Browse All under Users", async ({
  page,
}) => {
...
test("should display recent samples correctly", async ({ page }) => {
  ...
  await expect(
    page.getByRole("paragraph").filter({ hasText: "Recent Samples" })
  ).toBeVisible();

  // Verify that each recent sample is displayed
  for (const sample of recentSamplesMock.recentSamples) {
    await expect(
      page.getByRole("cell", { name: sample.code }).first()
    ).toBeVisible();
  }
});
