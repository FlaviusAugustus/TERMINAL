...
import { currentSamples, mockEntityDetails, mockSamples, mockSamplesNextPage, mockSearch, resetSamples, setCurrentData } from "./helpers/mocks";
import { sampleDetailsMock, sampleMock, samplesMock } from "./helpers/mockedData";
...
test("renders table with correct columns", async ({ page }) => {
  ...
  await samples.goto();
  await expect(page.getByRole('row', { name: 'Code', exact: true }).locator('div')).toBeVisible();
  await expect(page.getByRole('cell', { name: 'Project Name' }).locator('div')).toBeVisible();
  await expect(page.getByRole('cell', { name: 'Created At' }).locator('div')).toBeVisible();
...
test("searches for existing sample", async ({ page }) => {
  ...
  await mockSearch(page, "**/api/samples/search**", samplesMock.samples, "code", "samples");
...
test('should show sample details', async ({ page }) => {
  await mockSamples(page);
  await mockEntityDetails(page, SAMPLE_DETAILS_PATH, currentSamples, sampleDetailsMock, SAMPLE_ENTITY)
...
test("edits sample details", async ({ page }) => {
  ...
  await page.getByRole('row', { name: /Additional gases/i }).getByRole('combobox').selectOption('nitrogen');
  await page.getByRole('row', { name: /^Time/i }).getByRole('spinbutton').fill('1.5');
  await page.getByRole('row', { name: /^Pressure/i }).getByRole('spinbutton').fill('25');
  await page.getByRole('row', { name: /^Buffer/i }).getByRole('spinbutton').fill('100');
  await page.getByRole('row', { name: /^Substrate/i }).getByRole('combobox').selectOption('silicon dioxide');
  await page.getByRole('row', { name: /^CH₄/i }).getByRole('spinbutton').fill('99');
  await page.getByRole('row', { name: /^B₂H₆/i }).getByRole('spinbutton').fill('450');
  await page.getByRole('row', { name: /^H₂/i }).getByRole('spinbutton').fill('2000');
  await page.getByRole('row', { name: /^B\/C/i }).getByRole('spinbutton').fill('250');
  await page.getByRole('row', { name: /^Temperature/i }).getByRole('spinbutton').fill('720');
  await page.getByRole('row', { name: /^Pmw/i }).getByRole('spinbutton').fill('1200');


  // verify random two fields
  await expect(page.getByRole('row', { name: /^Time/i }).getByRole('spinbutton')).toHaveValue('1.5');
  await expect(page.getByRole('row', { name: /^Pressure/i }).getByRole('spinbutton')).toHaveValue('25');
...
test("paginates through samples list", async ({ page }) => {
  ...
  await page.locator('div:nth-child(3) > .sm\\:w-10\\/12 > .h-\\[40rem\\] > .flex.p-3 > .flex.gap-1 > button:nth-child(5)').click();
  await page.waitForTimeout(500);

  const secondPageFirstRow = await (await samples.getRow(1)).textContent();
  expect(firstPageFirstRow).not.toBe(secondPageFirstRow);

  await page.locator('div:nth-child(3) > .sm\\:w-10\\/12 > .h-\\[40rem\\] > .flex.p-3 > .flex.gap-1 > button:nth-child(4)').click();
  await page.waitForTimeout(500);
