...
test('should show no results for non-existing sample', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Search' }).click();
  await page.getByRole('textbox', { name: 'Search' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Search' }).fill('NonExistingSample123');
  await page.getByRole('textbox', { name: 'Search' }).press('Enter');
...
test('should paginate samples', async ({ page }) => {
  ...
  await page.locator('div:nth-child(3) > .sm\\:w-10\\/12 > .h-\\[40rem\\] > .flex.p-3 > .flex.gap-1 > button:nth-child(5)').click();
  const revertedFirstRowText = await page.locator('table tbody tr').first().textContent();
  expect(revertedFirstRowText).toBe(initialFirstRowText);
});

test('should edit sample and send correct step parameters', async ({ page }) => {
  ...
  await page.getByRole('row', { name: /Nucleation Method/i }).getByRole('combobox').selectOption('dip-coating');
...
test('should delete sample when clicking X button', async ({ page }) => {
  ...
  await expect(page.getByRole('row', { name: elementToDeleteContent || '' })).not.toBeVisible();
});
...
test('should delete selected samples', async ({ page }) => {
 ...
 page.getByRole('button', { name: /delete selected/i }).click();
  ...
  await expect(page.getByRole('row', { name: firstRowContent || '' })).not.toBeVisible();
  await expect(page.getByRole('row', { name: secondRowContent || '' })).not.toBeVisible();
});
